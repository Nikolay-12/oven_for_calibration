...
struct LevelStage {
    ...
    LevelStage() : level(0), hold_time(0) {}
    LevelStage(int lvl, float hold_t)
...
//                     stageCount(0), currentLevel(0) {}
    MultiStageDACcontroller(uint8_t mode, float* volts, int count, float holding_time,    
                        bool verbose_output,
                        void (*setLevel)(size_t level, bool save), void (*currentCallback)(), 
        ...
        stages = new LevelStage[count];
        for (int i = 0; i < count; i++) {
            int new_level = round(volts[i] / Vref * (float)MAX_VALUE);
            stages[i] = LevelStage(new_level, hold_time); // параметры: напряжение (В), время на выдержку (с)                                           
        }
    ...
    void reset_stages(const float* volts, int count) {
        ...
        for (int i = 0; i < count; i++) {
            int new_level = round(volts[i] / Vref * (float)MAX_VALUE);
            stages[i] = LevelStage(new_level, hold_time);
        }
        ...
        Serial.println("The stages of the multi-stage DAC have been updated!");
    }
    void reset_level_settings(uint8_t new_mode, int new_MIN_LEVEL, int new_MAX_LEVEL, int new_LEVEL_STEP) {
        if ((new_MAX_LEVEL - new_MIN_LEVEL) % new_LEVEL_STEP == 0) {
            ...
            Serial.println("Level settings have been updated!");
        }
        else {
            Serial.println("An error occurred while configuring the levels!");
            stage_count = 0;
            MIN_LEVEL = 0;
            MAX_LEVEL = 4095;
        }
    ...
    void reset_time_settings(float new_holding_time) {
        ...
        Serial.println("The holding time has been updated!");
    }
    ...
    void measurement_reset(int new_cycles_num, float new_measurement_period) {
        ...
        Serial.println("DAC controller measurement settings have been updated!");
    }
...
private:
    ...
    void updateCurrentLevel() {
        current_level = analogRead(DAC_OUTPUT_PIN);
    }
