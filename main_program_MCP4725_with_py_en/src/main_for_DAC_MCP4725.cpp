...
namespace DAC_MCP4725
{
  ...
  //  Команда для очищения стадий и удаления обработчика ЦАП
  void clear() {
  ...
  void init() {
    ...
    Serial.println("Multi-stage regulator for the MCP4725-based DAC initialized.");
    delay(500);
  }
  ...
  void updateSettings(const float* param) { // обновление базовых настроек ЦАП
    ...
    if ((Vmin < 0) || (Vmax > 3.3) || (Vmin >= Vmax) || (deltaV < 0.01) || (!isDivisible(Vmax-Vmin, deltaV)) || (holding_time <= 0)) { // for ESP32
      Serial.print("There is problem this the settings! ");
    ...
    Serial.print("New parameters: Vmin = "); Serial.print(Vmin); Serial.print(" V; Vmax = "); 
  ...
  void changeMode(const int new_mode) {
    if (new_mode == 0 || new_mode == 1 || new_mode == 2 || new_mode == 3) {
      mode = new_mode;
      Serial.print("Operation mode have been changed. New mode = "); Serial.println(mode);
    }
    else {
  ...
  int getLevel() { // Получение уровня сигнала от 0 до 4095
    return current_level;
  }
  void getData() { // Получение данных обо всех настройках ЦАПа
  ...
  bool getState() { // Возвращает состояние ЦАПа (true - включен, false выключен)
    return state;
  }
  uint8_t getMode() { // Возвращает режим работы ЦАП
    return mode;
  }

  // ====================================================
  //  ФУНКЦИЯ ДЛЯ НЕПОСРЕДСТВЕННОЙ РАБОТЫ С ЦАПом
  // ====================================================
