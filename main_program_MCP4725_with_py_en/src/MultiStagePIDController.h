...
private:
    ...
    // Для измерений
    float (*readAvrTempFunc)(); // Получает среднюю температуру с 25 термопар
    void (*readAllTempFunc)(); // Получает все температуры с 25 термопар
...
public:
    MultiStagePIDController(uint8_t mode, float* temperatures, int count, 
                           ...
                           bool verbose_output,
                           float (*tempCallback)(), void (*allTempCallback)(), void (*currentCallback)(), 
        ...
        verbose(verbose_output), // дополнительный параметр, который в состоянии true выдает дополнительную информацию о ходе
                                // работы
        readAvrTempFunc(tempCallback), // функция, вызываемая для измерения средней температуры
        readAllTempFunc(allTempCallback), // функция, вызываемая для измерения всех температур
    ...
    void stages_settings_reset(float* temperatures, int count) {
        ...
        Serial.println("The stages of the multi-stage temperature controller have been updated!");
        //Serial.println("The MULTI_STAGE mode is set by default.");
    }
    ...
    void temp_settings_reset(uint8_t new_mode, float new_Tmin, float new_Tmax, float new_deltaT) {
        ...
        Serial.println("Settings for the automatic creation of stages have been updated!");    
    }
    ...
    void PID_settings_reset(float new_kp, float new_ki, float new_kd,  float new_dt) {
        ...
        Serial.println("PID parameters have been changed!");
    }
    ...
    void time_and_tol_settings_reset(float new_stab_time, float new_hold_time, float new_tolerance, float max_time_per_stage) {
        ...
        reset_mode_state_timers();
        stability_checker.updateRequiredSamples(Dt, stability_time);
        Serial.println("Timing settings and temperature error have been updated!");
    }
    ...
    void measurement_reset(int new_cycles_num, float new_measurement_period) {
        ...
        Serial.println("The controller's measurement settings have been updated!");
    }
        ...
        switch (current_state) {
            case STABILIZATION: // работа на этапе стабилизации
                if (stability_checker.isStable()) {
                    ...
                    hold_start_time = now;
                    if (verbose) {
                        Serial.println("[LOG] Stabilization achieved!");
                    }
                }
                else if (elapsed_stage > max_stage_time) {
                    if (verbose) {
                        Serial.println("[LOG] Stage timeout triggered! Forced transition.");
                    }
                    current_state = State::HOLDING;
                    hold_start_time = now;
                }
            ...
            case HOLDING: { // работа на этапе выдержки
                if (now - hold_start_time >= hold_time * 1000) {
                    if (verbose) {
                        Serial.println("[LOG] The holding stage is complete.");
                    }
            ...
            case SWITCHING: // работа на этапе переключения
                if (current_stage + 1 < stage_count) {
                    ...
                    if (verbose) {
                        Serial.print("[LOG] Switching to the stage ");
    ...
    void updateMeasurements() {
        ...
        if ((now - last_measure_time) >= (measurement_period * 1000)) {
            ...
            if (temp_buffer && current_buffer && current_cycle < buffer_size) {
                temp_buffer[current_cycle] = readAvrTempFunc();
