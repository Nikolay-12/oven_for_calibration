...
namespace thermoRegulator
{
  ...
  void init() {
    ...
    Serial.println("25 thermocouples were initialized.");

    Serial.println("Multistage PID controller was initialized.");
  ...
  void updateStages(const char* data) { // обновление стадий в памяти контроллера
    ...
    if (stages_count != 0) {
      Serial.print("The size of the resulting float array: ");
      Serial.println(stages_count);
      Serial.print("Elements of the resulting float array: ");
      ...
      Serial.println("Error parsing the string containing the list of stages.");
      resetStages();
    }
    mode = 1;
    delay(100);
  ...
  void updateTempSettings(const float* param) { // обновление стадий, введенных пользователем через начало, конец и шаг
    ...
    if ((Tmin < 0) || (Tmax > 700) || (Tmin >= Tmax) || (deltaT < 1) || (!isDivisible(Tmax-Tmin, deltaT))){
      ...
      Serial.print("New parameters: Tmin = "); Serial.print(Tmin); Serial.print(" °C; Tmax = "); 
  ...
  void updatePIDcoef(const float* PID_parameters) {  // обновление параметров ПИД-регулятора
    ...
    Serial.print("New parameters: Kp = "); Serial.print(Kp); Serial.print("; Ki = "); Serial.print(Ki); 
  ...
  void updateTimeTolSettings(const float* time_parameters) { // обновление временных параметров
    if ((time_parameters[0] > 0) && (time_parameters[1] > 0) && (time_parameters[2] > 0) && (time_parameters[3] > 0)) {
      ...
      delay(100);
      Serial.print("New parameters: stabilization_time = "); Serial.print(stabilization_time); 
      ...
      Serial.println("Some parameters are set incorrectly. Check the settings and try again.");
    }
  }
  ...
  void changeMode(const int new_mode) {
    if (new_mode == 0 || new_mode == 1 || new_mode == 2 || new_mode == 3) {
      mode = new_mode;
      Serial.print("Operation mode have been changed. New mode = "); Serial.print(mode);
      switch (mode) {
        case 0:
  ...
  void on() {
    if (mode != 0) {
      ...
      DAC_MCP4725::setOutputLevel(VOLTAGE_MIN_FOR_IT_MODE, false);
      getData();
      state = true;
  ...
  void off() {
    ...
    DAC_MCP4725::setOutputLevel(0, false);
    SystemState::free();
  }
  ...
  void startTempReading() {
    if ((mode == 4) || (mode == 5)) { 
        ...
        Serial.print("Operation in mode "); Serial.print(mode); Serial.println(" is not supported!"); 
        isReading = false;
    }
