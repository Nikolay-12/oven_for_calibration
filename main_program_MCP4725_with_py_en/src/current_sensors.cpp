...
namespace current_sensors
{
    ...
    bool calibration_mode = 1; // По умолчанию должен вызываться для установки в режим измерения I(T)?
    ...
    void init(const descriptor_t descriptors[CURRENT_SENSORS_NUM])
    {
        ...
        for (size_t i = 0; i < CURRENT_SENSORS_NUM; i++)
        {
            ...
            if (ina219::isConnected(inst.d->address))
            {
                ...
                if (calibration_mode){
                    ...
                    ina219::setCalibration(inst.d->address, 
                    static_cast<uint16_t>(roundf(MY_INA219_CAL_MAGIC / inst.d->shunt_R2_ohms)), 
                    MY_INA219_CURRENT_LSB);
                }
        ...
        Serial.print("Initialized sensors: ");
    ...
    void init_by_address(const uint8_t addr, const float shunt_resistance_ohms, bool& initialization_ok) {
        if (ina219::isConnected(addr)) //Success: the device is present and ready to accept instructions
        {
            ...
            Serial.println("Current sensor INA219 initialized!");
        }
        else {
            Serial.println("Current sensor INA219 not initialized!");
        }
    }

    float get_current_amps_by_address(const uint8_t addr, const float shunt_resistance_ohms, bool& initialization_ok) {
        if (!initialization_ok) {
            Serial.println("Warning: Current sensor INA219 wasn't initialized!");
            return 0.0;
        }
    ...
    void set_calibration(bool new_mode) {
        ...
        if (calibration_mode) {
            Serial.println("Have the sensors been calibrated for I(T) mode measurements?");
        }
        else {
            Serial.println("Have the sensors been calibrated for I(V) mode measurements?");
        }
    }
