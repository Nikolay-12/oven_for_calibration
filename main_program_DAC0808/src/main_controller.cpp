...
namespace MainController 
{
    ...
    void inProgress() {
        if ((main_mode == Mode::IV_MODE) && (DAC0808::getState())) {
            DAC0808::inProgress(); // включение ЦАП
        }
        ...
        if ((new_mode == 0) || (new_mode == 1) || (new_mode == 2)) {
            ...
            switch (main_mode) {
                case Mode::IDLE:
                    ...
                    Serial.println("По умолчанию транзистор управления реле переведен в режим измерения I(T).");
                    Serial.println("По умолчанию проведена калибровка сенсоров INA219 для измерений в режиме I(T).");
                    break;
                case Mode::IT_MODE:
