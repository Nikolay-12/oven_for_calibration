...
namespace commandAnalyzer
{
    void analyzeCommand(ParsedCommand* cmd){
        ...
        if (strcmp(cmd->device_name, "MAX6675") == 0) {
            if (strcmp(cmd->main_key, "startTempReading") == 0) {  //"MAX6675:startTempReading;"
                thermoRegulator::startTempReading();
            }
            else if (strcmp(cmd->main_key, "stopTempReading") == 0) {  //"MAX6675:stopTempReading;"
                thermoRegulator::stopTempReading();
            }

            else if (strcmp(cmd->main_key, "readAvrTemp") == 0) {  //"MAX6675:readAvrTemp;"
                thermoRegulator::readAvrTemp();
            }
            else if (strcmp(cmd->main_key, "readAllTemp") == 0) {  //"MAX6675:readAllTemp;"
...
MAX6675:startTempReading;
MAX6675:stopTempReading;
MAX6675:readAvrTemp;
MAX6675:readAllTemp;
