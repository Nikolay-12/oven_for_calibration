...
private:
    ...
    void (*readAllTempFunc)(); // Получает все температуры с 25 термопар (+ среднюю)
...
public:
    MultiStagePIDController(uint8_t mode, float* temperatures, int count, 
                           ...
                           bool verbose_output,
                           float (*tempCallback)(), void (*allTempCallback)(), float (*currentCallback)(), 
        ...
        for (int i = 0; i < count; i++) {
            stages[i] = Stage(temperatures[i], stab_time, hold_time_s, tolerance);                                              
        }
    ...
    void stages_settings_reset(float* temperatures, int count) {
        ...
        stage_count = count;
        for (int i = 0; i < count; i++) {
            stages[i] = Stage(temperatures[i], stability_time, hold_time, tolerance);
        }
    ...
    void time_and_tol_settings_reset(float new_stab_time, float new_hold_time, float new_tolerance, float max_time_per_stage) {
        ...
        for (int i = 0; i < stage_count; i++) {
            stages[i] = Stage(stages[i].temperature, new_stab_time, new_hold_time, new_tolerance);
        }
