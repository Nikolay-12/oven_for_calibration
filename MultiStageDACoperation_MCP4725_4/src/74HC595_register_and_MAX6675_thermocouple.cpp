...
namespace register_74HC595
{
  ...
  void set_byte_in_reg(uint8_t value){ // The value is allowed to be from 0 to 255
    digitalWrite(HC595_STcp_PIN, LOW);  // Turn off the output (so that it doesn't blink)
    shiftOut(HC595_DS_PIN, HC595_SHcp_PIN, MSBFIRST, value); // Send the byte
    digitalWrite(HC595_STcp_PIN, HIGH); // Turn on the output — the data appears on the pins Q0–Q7
  }
  // 
  /*
