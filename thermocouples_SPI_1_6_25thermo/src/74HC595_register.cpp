...
void ShiftRegister74HC595::write(uint8_t data, bool latchEnabled = true) { // параметр latchEnabled (по умолчанию true для обратной совместимости) позволяет не защелкивать регистр
    if (latchEnabled) digitalWrite(_ST_pin, LOW);
    ...
    if (latchEnabled) digitalWrite(_ST_pin, HIGH);
    delayMicroseconds(10);
}

void ShiftRegister74HC595::writeRegisters(uint32_t data) {
    digitalWrite(_ST_pin, LOW);
    for (int i = REGISTERS_NUM - 1; i >= 0; i--) {
        uint8_t byteToSend = (data >> (i * 8)) & 0xFF;
        write(byteToSend, false);  // Без LATCH
    }
    digitalWrite(_ST_pin, HIGH);
}
