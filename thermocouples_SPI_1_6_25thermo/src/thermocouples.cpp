...
namespace thermocouples
{
  ...
  { 
    ...
    for (size_t i = 1; i <= THERMOCOUPLES_NUM; i++) {
        ...
        delayMicroseconds(50); // Время на переключение
    ...
    _reg->writeRegisters(0xFFFFFFFF); // Отключаем все
  ...
  void read_temp_from_one_thermocouple_once(uint8_t thermocouple_index) // 230 ms
  {
    ...
    uint32_t zeroPosition = 1 << thermocouple_index - 1 + FIRST_USEFUL_PIN - 1; // начинаем с соответствующей позиции "1" (0b...010...000000)
    uint32_t message = ~zeroPosition;
    _reg->writeRegisters(message);
  ...
  void send_temp_array(bool calibrate){
    ...
    // Функция для получения калиброванной температуры
    float get_calibrated_temp(uint8_t thermocouple_index) {
        ...
        // Применяем калибровку из константного массива
        return CALIBRATION[thermocouple_index - 1].k * raw_temp + CALIBRATION[thermocouple_index - 1].b;
    }
}
