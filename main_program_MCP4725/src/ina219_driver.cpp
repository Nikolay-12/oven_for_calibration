...
namespace ina219
{
    ...
    void setAveraging(uint8_t addr, uint8_t mode)
    {
        uint16_t Value = 0x399F; // Default value: 0x399F (12-bit, Continuous)
        // Reset and adjust the averaging:
        Value &= ~0x0078; // Clearing bits 3-6 
        Value |= (mode << 3); // Recording a new averaging value   
        Write16(addr, INA219_REG_CONFIG, Value);
    }
